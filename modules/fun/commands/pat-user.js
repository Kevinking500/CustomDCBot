...
/*
 * Thin adapter: the /pat run() reads its recipient from interaction.options.getUser('user').
 * We delegate to that exact run() with a proxy whose options.getUser returns the context-menu
 * targetUser, so the rendered pat (message + gif) is identical to the slash command, including
